...
namespace ft
{
	...
	class	RBTreeSet
	{
		public:
			...
			typedef typename ft::tree_iterator<const value_type, const node>		iterator;
			typedef typename ft::tree_iterator<const value_type, const node>		const_iterator;
			typedef typename ft::reverse_iterator<iterator>							reverse_iterator;
			typedef typename ft::reverse_iterator<const_iterator>					const_reverse_iterator;

			/*	CONSTRUCTORS	*/
			RBTreeSet(const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) : _al(alloc), _al_node(node_allocator()), _compare(comp), _size(0)
			{ 
			...
			ft::pair<iterator, bool> insert(value_type entry)
			{
				...
				else
				{
					...
					if (new_one->is_null == false && *new_one->content == entry)
					{
						// std::cout << "DEJA DEDANS" << std::endl;
